...
```text
per-IP+Host rate: DOH_RATE_LIMIT / second (default 5/s = 300 requests/minute), burst DOH_RATE_BURST (default 100)
...
The image has working defaults (defined once, in `content/entrypoint.sh`); no environment variable is required for the default deployment. The public proxy is the externally reachable rate limiter; MosDNS is kept behind its loopback listener and does not duplicate that per-client limiter.
...
| `DOH_RATE_LIMIT` | `5` | Per-client+Host request rate, requests/second (300 requests/minute sustained). |
| `DOH_RATE_BURST` | `100` | Per-client+Host burst allowance (up to 100 DNS lookups in a short burst). |
